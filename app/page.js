...
async function initialConvo() {
  const initConvoResp = await fetch(`http://localhost:3000/api/initialConvo?id=916043469552758784`, { cache: 'no-store' });
 const initConvo = initConvoResp.json();
...
export default function Chat() {
  ...
  async function setup(){
    console.log(await initialConvo());
    conversation = await initialConvo();
    conversationElement = await conversation.map((item, key) => (
