...
export default function ChatInput({ conversation, setConversation }) {
    ...
    return (
        <div className="bg-white border border-gray-300 shadow-md rounded-2xl w-full max-w-4xl p-4 mx-auto flex flex-row gap-4 items-center">
            ...
            <input onChange={handleSearchChange} value={content} className="overflow-y-scroll outline-none resize-none border-none w-full text-base leading-6" placeholder="Chat with Delta..." rows="1" />
            <div onClick={async () => {
            setContent('');
            await handleMessage(content);
            await Refresh(setConversation);            
        }
        } className="text-2x self-end w-6 h-6 m-1 text-gray-600 transition-colors cursor-pointer">
